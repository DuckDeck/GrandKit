...
public final class StarReview: UIControl {
    ...
    public var starFillColor = UIColor.blue { // 星星的填充颜色
        didSet {
            setNeedsDisplay()
        }
    }

    public var starBackgroundColor = UIColor.gray {
        didSet {
            setNeedsDisplay()
        }
    }
    ...
    fileprivate var offsetX: Float {
        ...
        return ratio > startReviewWidthScale ? Float(frame.width) / 2 - startReviewWidthScale / 2 * Float(frame.height) : 0.0 // 左边的空白处
    }

    fileprivate var offsetY: Float {
        return ratio < startReviewWidthScale ? (Float(frame.height) - starRadius) / 2 : 0.0 // 上面的空白处
    }
        ...
        didSet {
            ...
            if target != nil, event != nil {
                if event == UIControl.Event.valueChanged {
                    sendAction(selector!, to: target, for: nil)
                }
            }
        }
    }
    ...
    init() {
        ...
        starRadius = Float(frame.size.height) - Float(layer.borderWidth * 2)
        if ratio < startReviewWidthScale {
            starRadius = Float(frame.width) / startReviewWidthScale - Float(layer.borderWidth * 2)
        }
    }
...
//        print("offsetY:\(offsetY)")
        ...
        for s in 0...(starCount - 1) {
            ...
            var starCenter = CGPoint(x: CGFloat(x), y: (frame.height) / 2) // 第个星的中心
            if ratio > startReviewWidthScale {
                starCenter = CGPoint(x: CGFloat(x) + CGFloat(offsetX), y: frame.height / 2)
            }
